...
Vec2d getWindowSize()
{
    ...
    auto height = getAppConfig().window_simulation_height + getAppConfig().window_stats_width;
    return { width, height };
}
...
Vec2d Application::getEnvSize() const
{
    ...
    double size(getAppConfig().simulation_size);
    return { size, size };
}
...
//			getEnv().resetControls();
        ...
        //Extension
        case sf::Keyboard::U://augmenter la temperature de 1 degré (extensions)
            getAppEnv().changeTemperature(1);
            break;

        case sf::Keyboard::L://baisser la température de 1 degré (extensions)
            getAppEnv().changeTemperature(-1);
            break;
    ...
    //         auto pos = getursorPositionInView();
